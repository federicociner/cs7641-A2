...
@click.option('--tsp_n', default=100, help='TSP - Number of cities.')
@click.option('--ff_n', default=1000, help='FF - Number of elements.')
@click.option('--cp_n', default=1000, help='CP - Number of elements.')
@click.option('--cp_t', default=200, help='CP - Reward threshold.')
@click.option('--sa_cr', default=0.15, help='SA - Cooling rate.')
@click.option('--ga_p', default=50, help='GA - population.')
@click.option('--ga_ma', default=50, help='GA - mating rate.')
...
@click.option('--mm', default=0.1, help='MIMIC - m.')
def run(oa, tsp_n, ff_n, cp_n, cp_t, sa_cr, ga_p, ga_ma, ga_mu, ms, mk, mm):
    ...
    if oa == 'RHC':
        ...
        elapsed = time.clock() - start
        print(
            'MIMIC with samples {}, keep {} and m {} ran in {} seconds'.format(ms, mk, mm))
